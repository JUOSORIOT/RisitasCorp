...
namespace SBPScripts
{
    ...
    {
        public float stepJuan = 0.1f;
        public float currentJuanRPM = 0f;

        public float stepTest = 20f;
        public float currentTest = 0f;
        ...
        public string ip2 = "192.168.43.212";
        ...
        {
            receiveEndPoint2 = new IPEndPoint(IPAddress.Parse(ip2), receivePort2);
            receiveClient2 = new UdpClient(receivePort2);
            receiveQueue2 = Queue.Synchronized(new Queue());
            receiveThread2 = new Thread(new ThreadStart(ReceiveDataListener2));
        ...
        private void ReceiveDataListener()
        {
            while (true)
            {
                try
                {
                    ...
                    UInt32 rpm = BitConverter.ToUInt32(data, 0);
                    Debug.Log(rpm);
                    receiveQueue.Enqueue(rpm);
        ...
        void Update()
        {
            ...
            {
                currentJuanRPM = Mathf.Clamp(currentJuanRPM + stepJuan, 0f, 40f);
            ...
            if (Input.GetKeyDown(KeyCode.C) == true)
            {
                ...
                if (currentJuanRPM == 0) sprint = false;
            }
            ...
            if (receiveQueue.Count != 0)
            {
                ...
                currentJuanRPM = 30 * Mathf.InverseLerp(0, 300, Mathf.Clamp(value, 0, 300));
                Debug.Log("New target speed: " + currentJuanRPM.ToString());
            }
            topSpeed = currentJuanRPM;
        ...
        float CustomInput(string name, ref float axis, float sensitivity, float gravity, bool isRaw)
        {
            ...
            if (name == "Vertical")
            {
                r = Mathf.Clamp(currentJuanRPM, 0f, 1f);
            }
